...
from researchlab.design.core import Config, FieldSelector, Selector, State
...
def test_field_selector_functionality():
    ...
    selector = FieldSelector(val_x="state.x", val_y="config.y", val_z="state.nested.z")
...
def test_nested_state_selector():
    ...
    @FieldSelector(
        val_x="state.x", val_sub="state.sub.val", val_dict="state.d.k", val_tuple="state.t.0"
    )
    ...
    state = ComplexState(x=10, sub=sub, d={"k": 30}, t=(40, "ignored"))
    config = EmptyConfig()
